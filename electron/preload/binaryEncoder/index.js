...
function parseVersion(version = DEFAULT_FILE_VERSION) {
	invariant__default.default(REGEX_VERSION_STRING.test(version), 'file version data did not match expected format');
...
function parseRebusTable(tableString) {
	invariant__default.default(REGEX_REBUS_TABLE_STRING.test(tableString), 'Rebus table text doesn\'t match expected format.');
	return tableString
...
function parseExtensionSection(reader) {
	...
	invariant__default.default(checksum(data) === checksum_e, `"${title}" section data does not match checksum"`);
	invariant__default.default(NULL_BYTE.equals(sectionTerminator), `"${title}" section is missing terminating null byte`);
	return {title, data};
}
...
function validate(puzzle) {
	...
	invariant__default.default(height != null, 'Puzzle is missing required field: "height"');
	invariant__default.default(width != null, 'Puzzle is missing required field: "width"');
	invariant__default.default(solution != null, 'Puzzle is missing required field: "solution"');
	invariant__default.default(clues != null, 'Puzzle is missing required field: "clues"');
	if (fileVersion != null) {
		// VALIDATE FILE VERSION STRING
		invariant__default.default(REGEX_VERSION_STRING.test(fileVersion), 'FileVersion must match the supported format: #.#[#]');
	}
	// VALIDATE SOLUTION SIZE
	invariant__default.default(solution.length === width * height, `Puzzle width & height suggest solution should be ${width * height} characters long. Found ${solution.length} characters instead.`);
	// VALIDATE SOLUTION CONTENT
	invariant__default.default(REGEX_SOLUTION.test(solution), 'Puzzle solution may only contain ., :, alphanumeric characters and the following symbols: @, #, $, %, &, +, ?');
	// VALIDATE STATE SIZE
	if (state != null) {
		invariant__default.default(state.length === width * height, `Puzzle width & height suggest state should be ${width * height} characters long. Found ${state.length} characters instead.`);
		// VALIDATE STATE MATCHES SOLUTION
		invariant__default.default([...state].every((value, i) => REGEX_BLACK_SQUARE.test(value) === REGEX_BLACK_SQUARE.test(solution[i])), 'Black Squares in solution and state must match');
		// VALIDATE STATE CONTENT
		invariant__default.default(REGEX_STATE.test(state), 'Puzzle state may only contain -, ., :, alphanumeric characters and the following symbols: @, #, $, %, &, +, ?');
	}
	// VALIDATE CLUE COUNT
	const requiredClueCount = [...solution].reduce((acc, value, i) => {
		if (squareNeedsAcrossClue({solution, width}, i))
		{acc += 1;}
		if (squareNeedsDownClue({solution, width}, i))
		{acc += 1;}
	...
	invariant__default.default(requiredClueCount === clues.length, `Puzzle solution expects ${requiredClueCount} clues, but found ${clues.length} clues`);
	// VALIDATE MARKUP GRID
	if (markupGrid != null) {
		invariant__default.default(markupGrid.length === solution.length, `Markup grid should match puzzle solution in length. Expected length ${solution.length}, but got ${markupGrid.length}`);
		invariant__default.default(markupGrid.every((value) => value != null &&
            Object.keys(value).every((key) => squareMarkupKeys.includes(key))), 'Markup grid contains unsupported values');
	}
	// VALIDATE REBUS
	if (rebus != null) {
		if (rebus.grid != null) {
			invariant__default.default(rebus.grid.length === solution.length, `Rebus grid should match puzzle solution in length.  Expected length ${solution.length}, but got ${rebus.grid.length}`);
			const gridKeys = new Set(rebus.grid.filter((a) => a != null));
			gridKeys.forEach((key) => {
				let _a;
				invariant__default.default(typeof ((_a = rebus.solution) === null || _a === void 0 ? void 0 : _a[key]) === 'string', `Rebus grid references key that is not found in solution: "${key}"`);
			});
		}
		if (rebus.state != null) {
			invariant__default.default(rebus.state.length === solution.length, `Rebus state should match puzzle solution in length.  Expected length ${solution.length}, but got ${rebus.state.length}`);
		}
	}
	// VALIDATE SCRAMBLING
...
function isCorrect(puzzle, ignoreRebus = false) {
	let _a;
	validate(puzzle);
	invariant__default.default(!puzzle.isScrambled, 'Please unscramble the puzzle before checking correctness');
...
function parseBinaryFile(data) {
	...
	invariant__default.default(signatureIndex >= 2 /* FILE_SIGNATURE_START */, 'File does not appear to be an AcrossLite PUZ file');
	...
	for (let i = 0; i < numberOfClues; i += 1) {
		...
		invariant__default.default(clue != null, 'Failed to read clue from puzzle file');
		clues.push(clue);
	}
	// read notepad
	...
	while (reader.hasBytesToRead()) {
		...
		switch (title) {
			...
			case 'RUSR' /* REBUS_STATE */: {
				...
				const rebusState = rebusStateString.slice(0, -1).split('\x00');
				// Improve clarity for library consumers by deleting "empty" null markers
			...
			case 'LTIM' /* TIMER */: {
				...
				invariant__default.default(REGEX_TIMER_STRING.test(timerString), 'Timer data doesn\'t match expected format.');
	...
	invariant__default.default(getHeaderChecksum(puzzle) === headerChecksum, 'Header contents don\'t match checksum.  Please check that you are reading a valid PUZ file.');
	// validate file checksum
	invariant__default.default(getFileChecksum(puzzle) === fileChecksum, 'File contents don\'t match checksum (1).  Please check that you are reading a valid PUZ file.');
	// validate "ICHEATED" checksum
	invariant__default.default(iCheatedChecksum.equals(getICheatedChecksum(puzzle)), 'File contents don\'t match checksum (2).  Please check that you are reading a valid PUZ file.');
	return puzzle;
}

export function printBinaryFile(puzzle) {
...
function rebusKeyCharToNum(char) {
	...
	invariant.invariant(num != null, `encodeRebusKey: Encountered an invalid character "${char}"`);
...
function rebusKeyNumToChar(num) {
	...
	throw new invariant.InvariantError(`encodeRebusKey: Encountered an unsupported rebus key "${num}"`);
}
...
function assertSingleLineSection(tagName, sectionLines) {
	invariant__default.default(sectionLines.length === 1, `<${tagName}> section expects exactly one line. Found ${sectionLines.length} lines.`);
}
function parseTextFile(file) {
	...
	invariant__default.default(hasV1Signature || hasV2Signature, 'File does not appear to be an Across Lite puzzle description');
	...
	requiredTags.forEach((tag) => {
		...
		invariant__default.default(isTagPresent, `File is missing required tag: <${tag}>`);
	});
	const puzzle = {};
	let across;
	let down;
	...
	while (unreadLineQueue.length > 0) {
		...
		invariant__default.default(SECTION_TAG.test(sectionTag), `Couldn't parse file.  Expected section tag, but got: "${sectionTag}"`);
		...
		switch (sectionName) {
			...
			case SECTION.SIZE: {
				...
				invariant__default.default(SIZE_CONTENT.test(sizeString), `Puzzle size expected in the format "WIDTHxHEIGHT" (e.g. "15x15"). Received: "${sizeString}"`);
			...
			case SECTION.GRID: {
				...
				invariant__default.default(trimmedLines.every((line) => permittedSolutionContent.test(line)), '<GRID> section contains unsupported characters.');
			...
			case SECTION.REBUS: {
				invariant__default.default(fileVersion !== 0 /* V1 */, 'The <REBUS> tag is not supported in V1 text files.  Consider using the <ACROSS PUZZLE V2> file tag');
				let solution = puzzle.solution;
				invariant__default.default(solution != null, 'The <REBUS> tag is expected to come after <GRID> in puzzle descriptions');
...
function printTextFile(puzzle, indentation = '\t', lineEndingStyle = 'Unix') {
	...
	if ((rebus === null || rebus === void 0 ? void 0 : rebus.grid) != null && (rebus === null || rebus === void 0 ? void 0 : rebus.solution) != null) {
		...
		Object.entries(rebusSolution).forEach(([key, substitution]) => {
			...
			invariant__default.default(!Number.isNaN(numericKey), `Encoded rebus keys should be numeric. Found: ${key}`);
			...
			rebusGrid.forEach((key, i) => {
				if (key === numericKey) {
					...
					invariant__default.default(shortSolution === grid[i], 'Text format cannot encode multiple short solutions for a single rebus substitution');
				}
			});
			invariant__default.default(shortSolution != null && indices.length > 0, `Rebus solutions should have at least one corresponding grid entry. Key: ${key} Grid: ${rebusGrid}`);
...
// exports.getBlankState = getBlankState;
// exports.getFileChecksum = getFileChecksum;
// exports.getFileEncoding = getFileEncoding;
// exports.getHeaderChecksum = getHeaderChecksum;
// exports.getICheatedChecksum = getICheatedChecksum;
// exports.getState = getState;
// exports.gridNumbering = gridNumbering;
// exports.hasRebusSolution = hasRebusSolution;
// exports.hasRebusState = hasRebusState;
// exports.isCorrect = isCorrect;
// exports.parseBinaryFile = parseBinaryFile;
// exports.parseTextFile = parseTextFile;
// exports.printBinaryFile = printBinaryFile;
// exports.printTextFile = printTextFile;
